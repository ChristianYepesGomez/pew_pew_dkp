...
export async function autoCloseAuction(db, auctionId) {
  try {
    ...
    const closeResult = await db.transaction(async (tx) => {
      // Re-check status inside the transaction to prevent double-close from concurrent
      // timeout + sweep firing at the same time (race condition guard).
      const fresh = await tx.get('SELECT status FROM auctions WHERE id = ?', auctionId);
    ...
    if (closeResult === null) {
      log.info(`Auction ${auctionId} already closed by concurrent call — skipping`);
      return;
    }
