...
const configCacheMap = new Map(); // db -> { values, lastFetch }

function getOrCreateCache(db) {
  if (!configCacheMap.has(db)) {
    configCacheMap.set(db, { values: {}, lastFetch: 0 });
  }
  return configCacheMap.get(db);
}

export async function getCachedConfig(db, key, defaultValue) {
  const cache = getOrCreateCache(db);
  const now = Date.now();
  if (now - cache.lastFetch > CONFIG_CACHE_TTL) {
    try {
      const configs = await db.all('SELECT config_key, config_value FROM dkp_config');
      cache.values = {};
      for (const c of configs) {
        cache.values[c.config_key] = c.config_value;
      }
      cache.lastFetch = now;
    } catch (e) {
...
export function invalidateConfigCache(db) {
  if (db) {
    const cache = configCacheMap.get(db);
    if (cache) cache.lastFetch = 0;
  } else {
    // Invalidate all caches
    for (const cache of configCacheMap.values()) {
      cache.lastFetch = 0;
    }
  }
}
...
export function getCurrentRaidWeek() {
  ...
  const weekNumber = Math.ceil(((wednesday.getTime() - startOfYear.getTime()) / 86400000 + startOfYear.getDay() + 1) / 7);
