  ...
  return (
    <div className="absolute left-full top-0 bottom-0 pl-3 w-44 z-10">
      ...
      >
        ...
        {/* Bleacher rows — staggered. overflow-visible so speech bubbles aren't clipped. */}
        <div className="flex-1 px-2 py-2 flex flex-col gap-2" style={{ overflow: 'visible' }}>
          {rows.map((row, rowIdx) => (
...
function Seat({ player }) {
  ...
  const { text, visible } = useSpeechBubble(STANDS_PHRASES, { minDelay: 5000, maxDelay: 25000 })

  return (
    <div
      title={`${player.character_name}${player.spec ? ` · ${player.spec}` : ''}`}
      className="relative flex flex-col items-center gap-0.5 opacity-70"
      style={{ overflow: 'visible' }}
    >
      {visible && <SpeechBubbleAbove text={text} visible={visible} />}
      <div
